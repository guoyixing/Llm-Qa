...
- `b66b2994c9f3aa2b394e8a79b183b9bd0adecd5b`
- `64a99dc64989232d7f6f5c929820c8ddc193923c`
...
- 项目：`hisense-ids-app`
- 提交：`64a99dc64989232d7f6f5c929820c8ddc193923c`
- 类别：代码规范
- 严重级别：LOW
- 文件与行范围：`hisense-app/src/main/java/com/glaway/structureddelivery/service/impl/MileStonePlanBizServiceImpl.java:142-180`
- 规则或代码证据：公共规则 `STYLE-003`、`ROBUST-004`。`sortByTemplate` 使用空的 `catch (Exception e) {}` 捕获模板查询、Bean 获取和排序路径中的所有异常，随后直接返回原列表。
- 问题说明：异常既未转换、未保留根因，也没有明确的降级状态。
- 影响：调用方无法区分“按模板排序成功”和“排序失败后返回原顺序”，问题也无法从诊断信息定位。
- 建议：缩小捕获范围；若允许降级，记录稳定项目标识和异常根因并明确降级语义；若排序属于结果契约，则返回可识别失败。
